...
template<class TGeom>
void
GeoElementTemplate<TGeom>::Jacobian(const Matrix &gradx, Matrix &jac, Matrix &axes, double &detjac, Matrix &jacinv) {
    detjac = 0.0;
    ...
    switch (dim) {
        ...
        default: {
            std::cout << "Object with wrong dimensions, unable to compute jacobian matrix. Dimension = " << dim
                      << std::endl;
            DebugStop();
            break;
        }
...
template
class GeoElementTemplate<Geom1d>;

template
class GeoElementTemplate<GeomQuad>;

template
class GeoElementTemplate<GeomTriangle>;

template
class GeoElementTemplate<GeomTetrahedron>;
